...
% starting time of the NPF event
...
% c_am=[c_a/Avogado.*M(acids) c_n/Avogado.*M(neutrals)]; %Mass concentrations (kg/m3)
...
%     [tout, output, exitflag1] = ode23s(@flx_eaim_MABNAG2_Bolivia,starttimes,input(whats<0 | whats>=10),options,whats);
     [tout, output, exitflag1] = ode15s(@flx_eaim_MABNAG2_Bolivia,starttimes,input(whats<0 | whats>=10),options,whats);
